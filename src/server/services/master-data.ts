...
import { supplierSchema, orgSchema, taxSubjectSchema, invoiceItemSchema, taxCodeSchema } from "@/schemas/master-data";
...
async function crudList(entity: string, search?: string, searchFields?: string[]) {
  ...
  const items = await (db as unknown as Record<string, { findMany: (a: unknown) => Promise<unknown> }>)[entity]!.findMany({ where, orderBy: { createdAt: "desc" }, take: 100 });
...
async function crudCreate(entity: string, data: Record<string, unknown>, orgId: string) {
  return (db as unknown as Record<string, { create: (a: { data: unknown }) => Promise<unknown> }>)[entity]!.create({ data: { ...data, organizationId: orgId } });
}

async function crudUpdate(entity: string, id: string, data: Record<string, unknown>) {
  return (db as unknown as Record<string, { update: (a: { where: { id: string }; data: unknown }) => Promise<unknown> }>)[entity]!.update({ where: { id }, data });
}

async function crudDelete(entity: string, id: string) {
  return (db as unknown as Record<string, { delete: (a: { where: { id: string } }) => Promise<unknown> }>)[entity]!.delete({ where: { id } });
}
...
function createService<T>(entity: string, schema: ZodSchema<T>, searchFields: string[]) {
  return {
    ...
    getById: (id: string) => (db as unknown as Record<string, { findUnique: (a: { where: { id: string } }) => Promise<unknown> }>)[entity]!.findUnique({ where: { id } }),
    create: async (input: unknown, userId: string) => {
      ...
      try { await db.operationLog.create({ data: { userId, action: "CREATE", entityType: entity, entityId: (item as { id: string }).id } }); } catch { /* log failure should not block the API */ }
      return { success: true as const, data: item };
    },
    update: async (id: string, input: unknown, userId: string) => {
      const r = validate((schema as unknown as { partial: () => ZodSchema<T> }).partial(), input);
      if (!r.success) return r;
      const item = await crudUpdate(entity, id, r.data as Record<string, unknown>);
      try { await db.operationLog.create({ data: { userId, action: "UPDATE", entityType: entity, entityId: id } }); } catch { /* log failure should not block the API */ }
      return { success: true as const, data: item };
    },
    delete: async (id: string, userId: string) => {
      await crudDelete(entity, id);
      try { await db.operationLog.create({ data: { userId, action: "DELETE", entityType: entity, entityId: id } }); } catch { /* log failure should not block the API */ }
