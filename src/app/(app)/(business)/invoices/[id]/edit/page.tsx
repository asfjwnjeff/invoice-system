...
export default function InvoiceEditPage() {
  ...
  const [f, setF] = useState({
    ...
    issueDate: "", businessDate: "", businessDescription: "",
    amountWithoutTax: 0, taxAmount: 0, amountWithTax: 0, currency: "CNY", remark: "",
    deliveryStatus: "PENDING", verifyStatus: "PENDING", verifiedBy: "", verifyTime: "",
    headerValidationStatus: "PENDING", headerValidationMessage: "",
    usageStatus: "UNUSED", pushStatus: "PENDING", accountStatus: "UNACCOUNTED", archiveStatus: "UNARCHIVED",
  ...
  useEffect(() => {
    ...
    setF({
      ...
      businessDescription: inv.businessDescription ?? "",
      amountWithoutTax: Number(inv.amountWithoutTax ?? 0), taxAmount: Number(inv.taxAmount ?? 0),
      amountWithTax: Number(inv.amountWithTax ?? 0), currency: inv.currency ?? "CNY", remark: inv.remark ?? "",
      deliveryStatus: inv.deliveryStatus ?? "PENDING", verifyStatus: inv.verifyStatus ?? "PENDING",
      verifiedBy: inv.verifiedBy ?? "", verifyTime: inv.verifyTime?.slice(0, 16) ?? "",
        ...
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-2"><Label>所属票池</Label>
            <Select value={f.invoicePool} onValueChange={v => setF({...f, invoicePool: v})}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="OUTPUT">OUTPUT</SelectItem><SelectItem value="INPUT">INPUT</SelectItem></SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2"><Label>录入方式</Label>
            <Select value={f.entryMethod} onValueChange={v => setF({...f, entryMethod: v})}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="MANUAL">手工录入</SelectItem><SelectItem value="SYSTEM">系统生成</SelectItem>
                <SelectItem value="OCR">OCR识别</SelectItem><SelectItem value="IMPORT">批量导入</SelectItem>
                <SelectItem value="PC_UPLOAD">PC端上传</SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="space-y-2"><Label>交付状态</Label>
            <Select value={f.deliveryStatus} onValueChange={v => setF({...f, deliveryStatus: v})}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="PENDING">待交付</SelectItem><SelectItem value="DELIVERED">已交付</SelectItem><SelectItem value="FAILED">交付失败</SelectItem></SelectContent>
            </Select>
          </div>
          <div className="space-y-2"><Label>使用状态</Label>
            <Select value={f.usageStatus} onValueChange={v => setF({...f, usageStatus: v})}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="UNUSED">未使用</SelectItem><SelectItem value="USED">已使用</SelectItem><SelectItem value="RED_FLUSHED">已红冲</SelectItem><SelectItem value="VOIDED">已作废</SelectItem><SelectItem value="ARCHIVED">已归档</SelectItem></SelectContent>
            </Select>
          </div>
          <div className="space-y-2"><Label>推送状态</Label>
            <Select value={f.pushStatus} onValueChange={v => setF({...f, pushStatus: v})}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="PENDING">待推送</SelectItem><SelectItem value="PUSHED">已推送</SelectItem></SelectContent>
            </Select>
          </div>
          <div className="space-y-2"><Label>入账状态</Label>
            <Select value={f.accountStatus} onValueChange={v => setF({...f, accountStatus: v})}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="UNACCOUNTED">未入账</SelectItem><SelectItem value="ACCOUNTED">已入账</SelectItem></SelectContent>
            </Select>
          </div>
          <div className="space-y-2"><Label>归档状态</Label>
            <Select value={f.archiveStatus} onValueChange={v => setF({...f, archiveStatus: v})}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="UNARCHIVED">未归档</SelectItem><SelectItem value="ARCHIVED">已归档</SelectItem></SelectContent>
            </Select>
          </div>
        </CardContent>
      </Card>
