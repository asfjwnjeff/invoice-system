...
export default function ApplicationNewPage() {
  ...
  const loadTaxSubject = async (id: string) => {
    setF(p => ({ ...p, taxSubjectId: id }));
    setBankAccounts([]);
    if (!id) return;
    const r = await fetch(`/api/tax-subjects/${id}`);
    const j = await r.json();
    if (j.success && j.data) {
    ...
    if (bj.success && bj.data?.items) {
      const items = bj.data.items as { id: string; label: string; bankName: string; bankAccount: string; isDefault: boolean }[];
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      const j = await r.json();
      if (!r.ok || !j.success) throw new Error(j.error || "创建失败");
      return j;
    },
    onSuccess: () => { toast.success("开票申请已创建"); router.push("/applications"); },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <Card className="lg:col-span-3">
          ...
          <CardContent className="grid grid-cols-2 gap-x-8 gap-y-6">
            ...
            <FormField label="购方纳税人识别号" fullWidth>
            ...
            <FormField label="购方开户行">
              <Input value={f.buyerBankName} onChange={e => setF({ ...f, buyerBankName: e.target.value })} placeholder="开户行" />
            </FormField>
            <FormField label="购方银行账号">
              <Input value={f.buyerBankAccount} onChange={e => setF({ ...f, buyerBankAccount: e.target.value })} placeholder="银行账号" />
            </FormField>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card className="lg:col-span-3">
          ...
          <CardContent className="space-y-4">
            <FormField label="销方名称（搜索选择）" required>
              <SelectSearch
                value={f.taxSubjectId}
                onValueChange={loadTaxSubject}
                options={taxSubjectOptions.map(o => ({ value: o.id, label: o.label }))}
                placeholder="搜索选择税号主体..."
              />
            </FormField>
