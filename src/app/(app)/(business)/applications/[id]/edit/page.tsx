...
export default function ApplicationEditPage() {
  ...
  const [bankAccounts, setBankAccounts] = useState<{ id: string; label: string; bankName: string; bankAccount: string; isDefault: boolean }[]>([]);

  const { data: app, isLoading } = useQuery({ queryKey: ["application", id], queryFn: async () => { const r = await fetch(`/api/applications/${id}`); const j = await r.json(); return j.data as Record<string, unknown>; } });

  useEffect(() => {
    if (app) {
      setF({ applicationNo: String(app.applicationNo ?? ""), invoiceCategory: String(app.invoiceCategory ?? "DIGITAL_SPECIAL"), taxRate: +(((app.items as Record<string,unknown>[])?.[0]?.taxRate) || 6), customerId: String(app.customerId ?? ""), buyerName: String(app.buyerName ?? ""), buyerTaxNo: String(app.buyerTaxNo ?? ""), buyerAddressPhone: String(app.buyerAddress ?? ""), buyerBankName: String(app.buyerBankName ?? ""), buyerBankAccount: String(app.buyerBankAccount ?? ""), taxSubjectId: String(app.taxSubjectId ?? ""), sellerName: String(app.sellerName ?? ""), sellerTaxNo: String(app.sellerTaxNo ?? ""), sellerAddressPhone: String(app.sellerAddress ?? ""), sellerBankName: String(app.sellerBankName ?? ""), sellerBankAccount: String(app.sellerBankAccount ?? ""), currency: String(app.currency ?? "CNY"), cashierName: String(app.cashierName ?? ""), reviewerName: String(app.reviewerName ?? ""), drawerName: String(app.drawerName ?? ""), remark: String(app.remark ?? ""), amountWithoutTax: Number(app.amountWithoutTax ?? 0), taxAmount: Number(app.taxAmount ?? 0), amountWithTax: Number(app.amountWithTax ?? 0) });
      const appItems = (app.items as Record<string, unknown>[]) ?? [];
      if (appItems.length > 0) { setItems(appItems.map((it: Record<string, unknown>) => ({ itemName: String(it.itemName ?? ""), taxClassificationCode: String(it.taxClassificationCode ?? ""), spec: String(it.spec ?? ""), unit: String(it.unit ?? "月"), quantity: Number(it.quantity ?? 1), unitPriceWithTax: Number(it.unitPrice ?? 0), amount: Number(it.amount ?? 0), taxRate: Number(it.taxRate ?? 6), taxAmount: Number(it.taxAmount ?? 0) }))); }
      const tsId = String(app.taxSubjectId ?? "");
      if (tsId) { fetch(`/api/tax-subjects/${tsId}/bank-accounts`).then(r => r.json()).then(j => { if (j.success && j.data?.items) { setBankAccounts(j.data.items); const saved = j.data.items.find((ba: { bankName: string; bankAccount: string }) => ba.bankName === String(app.sellerBankName ?? "") && ba.bankAccount === String(app.sellerBankAccount ?? "")); if (saved) setF(p => ({ ...p, sellerBankName: saved.bankName, sellerBankAccount: saved.bankAccount })); } }); }
    }
  }, [app]);
  ...
  const updateItem = (i: number, field: keyof Item, val: string | number) => { const next = [...items]; const it = { ...next[i]!, [field]: val }; it.amount = +((+it.unitPriceWithTax * +it.quantity) / (1 + +it.taxRate / 100)).toFixed(2); it.taxAmount = +(it.amount * +it.taxRate / 100).toFixed(2); next[i] = it; setItems(next); recalcTotals(next); };

  const loadCustomer = async (cid: string) => { setF(p => ({ ...p, customerId: cid })); if (!cid) return; const r = await fetch(`/api/customers/${cid}`); const j = await r.json(); if (j.success && j.data) { const c = j.data; setF(p => ({ ...p, buyerName: c.name || "", buyerTaxNo: c.taxNo || "", buyerAddressPhone: [c.address, c.phone].filter(Boolean).join(" "), buyerBankName: c.bankName || "", buyerBankAccount: c.bankAccount || "" })); } };
  const loadTaxSubject = async (tsid: string) => { setF(p => ({ ...p, taxSubjectId: tsid })); setBankAccounts([]); if (!tsid) return; const r = await fetch(`/api/tax-subjects/${tsid}`); const j = await r.json(); if (j.success && j.data) { const ts = j.data; setF(p => ({ ...p, sellerName: ts.name || "", sellerTaxNo: ts.taxNo || "", sellerAddressPhone: [ts.address, ts.phone].filter(Boolean).join(" ") })); } const br = await fetch(`/api/tax-subjects/${tsid}/bank-accounts`); const bj = await br.json(); if (bj.success && bj.data?.items) { setBankAccounts(bj.data.items); const def = bj.data.items.find((ba: { isDefault: boolean }) => ba.isDefault); if (def) setF(p => ({ ...p, sellerBankName: def.bankName, sellerBankAccount: def.bankAccount })); } };
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <Card><CardHeader className="pb-3 flex flex-row items-center justify-between"><CardTitle className="text-base">项目明细</CardTitle><Button type="button" variant="outline" size="sm" onClick={addItem}><Plus className="h-3.5 w-3.5 mr-1" />添加行</Button></CardHeader>
        <CardContent className="space-y-3">
          ...
          {items.map((item, i) => (<div key={i} className="grid grid-cols-12 gap-2 p-3 border rounded-sm bg-muted/50 items-center"><div className="col-span-3"><Input placeholder="项目名称" value={item.itemName} onChange={e => updateItem(i, "itemName", e.target.value)} /></div><div className="col-span-2"><Input placeholder="规格型号" value={item.spec} onChange={e => updateItem(i, "spec", e.target.value)} /></div><Input placeholder="单位" value={item.unit} onChange={e => updateItem(i, "unit", e.target.value)} /><Input type="number" value={item.quantity} onChange={e => updateItem(i, "quantity", +e.target.value)} /><div className="col-span-2"><Input type="number" step="0.01" value={item.unitPriceWithTax || ""} onChange={e => updateItem(i, "unitPriceWithTax", +e.target.value)} /></div><div className="col-span-2 text-sm tabular-nums text-right px-1">¥{item.amount.toLocaleString()}</div><div className="flex items-center gap-1"><span className="text-sm tabular-nums">¥{item.taxAmount.toLocaleString()}</span><Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={() => removeItem(i)} disabled={items.length <= 1}><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button></div></div>))}
