...
interface ApplicationData {
  ...
  cashierName: string; reviewerName: string; drawerName: string;
...
export default function ApplicationDetailPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => { const r = await fetch(`/api/applications/${id}`); return (await r.json()).data as ApplicationData; },
  });
    ...
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-semibold">开票申请详情</h1>
          <p className="text-sm text-muted-foreground">{S(data.applicationNo)}</p>
        </div>
        <StatusBadge status={appStatusLabels[S(data.status)] ?? S(data.status)} variant={sv(S(data.status))} />
        {data.status === "DRAFT" && (
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-4 gap-x-8 gap-y-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">未税合计</span><strong className="tabular-nums">¥{N(data.amountWithoutTax).toLocaleString()}</strong></div>
              <div className="flex justify-between"><span className="text-muted-foreground">税额合计</span><strong className="tabular-nums">¥{N(data.taxAmount).toLocaleString()}</strong></div>
              <div className="flex justify-between border-t pt-2"><span className="text-muted-foreground">价税合计</span><strong className="tabular-nums text-base">¥{N(data.amountWithTax).toLocaleString()}</strong></div>
            </div>
            <div className="mt-4"><DetailField label="记账金额（未税）" value={`¥${N(data.amountWithoutTax).toLocaleString()}`} /></div>
          </CardContent>
        </Card>
      </div>

      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-base">项目明细</CardTitle></CardHeader>
        <CardContent>
          {items.length > 0 ? (
          ...
          <div className="flex justify-end gap-6 text-sm pt-3 border-t mt-3">
            <span>未税合计: <strong className="tabular-nums">¥{N(data.amountWithoutTax).toLocaleString()}</strong></span>
            <span>税额合计: <strong className="tabular-nums">¥{N(data.taxAmount).toLocaleString()}</strong></span>
            <span>价税合计: <strong className="tabular-nums">¥{N(data.amountWithTax).toLocaleString()}</strong></span>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <DetailField label="收款人" value={S(data.cashierName)} />
            <DetailField label="复核人" value={S(data.reviewerName)} />
            <DetailField label="开票人" value={S(data.drawerName)} />
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base">来源与时间</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-x-8 gap-y-4">
            <DetailField label="来源类型" value={S(data.sourceType) || "手工"} />
            {!!data.revenueOrder && <DetailField label="收入订单" value={S(data.revenueOrder.orderNo)} />}
            <DetailField label="创建时间" value={data.createdAt ? new Date(S(data.createdAt)).toLocaleString("zh-CN") : "-"} />
            <DetailField label="更新时间" value={data.updatedAt ? new Date(S(data.updatedAt)).toLocaleString("zh-CN") : "-"} />
          </CardContent>
        </Card>
      </div>
    </div>
