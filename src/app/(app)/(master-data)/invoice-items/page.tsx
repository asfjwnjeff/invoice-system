...
const feeTypeLabels: Record<string, string> = { CUSTOMS_SERVICE: "关务服务", WAREHOUSE_SERVICE: "仓储服务", TRANSPORT_SERVICE: "运输服务", AGENCY_SERVICE: "代理服务" };
...
export default function Page() {
  ...
  const cols: ColumnDef<T>[] = [
    ...
    { id: "act", header: "操作", cell: ({ row }) => (<div className="flex gap-1"><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditId(row.original.id); setF({ name: row.original.name, taxCode: row.original.taxCode ?? "", unit: row.original.unit ?? "", feeType: row.original.feeType ?? "" }); setOpen(true); }}><Pencil className="h-3.5 w-3.5" /></Button><Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => setDeleteId(row.original.id)}><Trash2 className="h-3.5 w-3.5" /></Button></div>) },
  ];
  return (
    <div>
      ...
      <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>{editId ? "编辑" : "新增"}</DialogTitle></DialogHeader>
        <div className="grid grid-cols-2 gap-4 py-4">
          ...
          <div className="col-span-2 space-y-2"><Label>费用类型</Label><Input value={f.feeType} onChange={(e) => setF({ ...f, feeType: e.target.value })} placeholder="关务服务/仓储服务等" /></div>
        </div>
